...
class WatchModule {
  static watch(serverHelper) {
        ...
        timeoutSrc = setTimeout(async () => {
          try {
            ...
            if (MarkoModule.match(targetPath) === true) {
              MarkoModule.processFile(
                targetPath,
                Config.Path.Dev.Frontend.PAGES
              );
              modified = true;
            ...
            await DevGeneratedModule.matchAndProcess(
              targetPath,
              Config.Path.Dev
            ); // no need to restart server from here, because it is triggered from either webpack or RootModule
    ...
    watch(
      path.join(Config.Path.LANG, "txt"),
      {
        recursive: true,
      },
      (event, filename) => {
