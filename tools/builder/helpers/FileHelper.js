...
class FileHelper {
  ...
  static traversePath(
    ...
    for (let i = items.length; i-- > 0; ) {
      ...
      } else if (sourcePath !== Config.Path.ROOT) {
        // files in root folder should never be processed for now
  ...
  static isException(exception, targetPath) {
    ...
    for (let i = includeList.length; i-- > 0; ) {
      if (
        targetPath.indexOf(includeList[i]) === 0 ||
        includeList[i].indexOf(targetPath) === 0
      ) {
        return false;
      }
    }
