...
class ProdTarget {
  static async run() {
    ...
    fs.copyFileSync(
      ...
      path.join(Config.Path.PROD, "package.json")
    );
    fs.copyFileSync(
      path.join(Config.Path.ROOT, "package-lock.json"),
      path.join(Config.Path.PROD, "package-lock.json")
    );
    FileHelper.traversePath(
      path.join(Config.Path.PROD, "config"),
      "",
      null,
      false,
      sourceChildPath => {
