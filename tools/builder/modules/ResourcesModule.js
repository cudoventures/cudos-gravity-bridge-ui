...
class ResourcesModule {
  static async process(targetPath) {
    ...
    for (let i = items.length; i-- > 0; ) {
      ...
      exception = {
        ...
        exclude: [
          path.join(sourceChildPath, "css"),
          path.join(sourceChildPath, "js"),
        ],
      };
  ...
  static match(targetPath) {
    ...
    if (
      targetPath.indexOf("css") !== -1 &&
      targetPath.indexOf("inline") === -1
    ) {
      return false;
    }
  ...
  static processFile(sourcePath) {
    const targetPath = sourcePath.replace(
      Config.Path.Root.Frontend.RESOURCES,
      Config.Path.Dev.Frontend.RESOURCES
    );
