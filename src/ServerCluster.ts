...
class ServerCluster {
      ...
      switch (process.env.PROCESS_NAME) {
        case SERVER_WORKER_NAME:
          ...
          new ServerWorker(
            ...
            Config.Server.SESSION_UNIQUE_KEY,
            this.dbPool
          ).start();
          break;
        default:
  ...
  clusterOnWorkerExit = async (worker, code, signal) => {
    if (code === null) {
      Logger.log(
        `Worker ${worker.process.pid} restarted with code: ${code} and signal: ${signal} (${worker.process.env.PROCESS_NAME})`
      );
    } else {
      Logger.error(
        `Worker ${worker.process.pid} died with code: ${code} and signal: ${signal} (${worker.process.env.PROCESS_NAME})`
      );
    }

    if (
      worker.exitedAfterDisconnect !== true &&
      Config.Build.PRODUCTION === true
    ) {
