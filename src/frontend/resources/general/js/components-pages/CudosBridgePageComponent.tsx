...
export default class CudosBridgeComponent extends ContextPageComponent < Props, State > {
    ...
    async getContractBalance(): Promise<BigNumber> {
        try {
            ...
            return (new BigNumber(balance)).div(`1${'0'.repeat(CosmosNetworkH.CURRENCY_DECIMALS)}`);
        } catch (e) {
            console.log(e);
    ...
    onChangeAmount = (amount: string) => {
        ...
        this.inputTimeouts.amount = setTimeout(() => {
            ...
            if (bigAmount.isNaN() || bigAmount.isLessThan(new BigNumber(1).dividedBy(`1${'0'.repeat(CosmosNetworkH.CURRENCY_DECIMALS)}`)) || bigAmount.isGreaterThan(BigNumber.minimum(this.state.walletBalance, this.state.contractBalance))) {
                this.setState({
