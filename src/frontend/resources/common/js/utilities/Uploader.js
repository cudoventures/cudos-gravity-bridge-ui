...
export default class Uploader {
  ...
  static newInstance(params) {
    if (Uploader.isSupported() === false) {
      console.error(
        "Browser is  not suported. Please update to latest version of chrome/firefox/opera/safari or Internet Explorer 10 or newer"
      );
      return null;
    }
    ...
    if (params.controller === undefined) {
      console.error(
        "You have to set 'controller' attribute in Uploader constructor"
      );
      return null;
    }
...
class UploaderImpl {
  constructor(params) {
    ...
    if (params.maxSize === undefined) {
      console.warn(
        "It is highly recommended to set 'maxSize' attribute of Uploader to be equal to the value on the server. Now Uploader will use default value of 2097152 bytes(2MB)."
      );
      this.maxSize = 1 << 21;
    } else {
    ...
    this.progressWindow =
      params.progressWindow !== undefined ? params.progressWindow : true;
    this.uploadLabel =
      params.label !== undefined ? params.label : "Uploading...";
    this.dataParamName =
      params.dataParamName !== undefined ? params.dataParamName : "file";
    this.filenameParamName =
      params.filenameParamName !== undefined
        ? params.filenameParamName
        : "filename";
    this.typeParamType =
      params.typeParamType !== undefined ? params.typeParamType : "type";
    this.bgColor = params.bgColor !== undefined ? params.bgColor : "#a2a2a2";
    this.bgColorIndicator =
      params.bgColorIndicator !== undefined
        ? params.bgColorIndicator
        : "#a2a2a2";
    this.bgColorCard =
      params.bgColorCard !== undefined ? params.bgColorCard : "#fff";

    this.onUpload = params.onUpload !== undefined ? params.onUpload : null;
    this.onError = params.onError !== undefined ? params.onError : null;
    this.onBeforeSend =
      params.onBeforeSend !== undefined ? params.onBeforeSend : null;
    this.onProgress =
      params.onProgress !== undefined ? params.onProgress : null;
    this.onExceedLimit =
      params.onExceedLimit !== undefined ? params.onExceedLimit : null;
    this.onBeforeStart =
      params.onBeforeStart !== undefined ? params.onBeforeStart : null;
    this.onFinish = params.onFinish !== undefined ? params.onFinish : null;
    this.onExtError =
      params.onExtError !== undefined ? params.onExtError : null;

    if (this.inputN !== null) {
      if (params.position !== undefined) {
        this.inputN.parentNode.style.position = params.position;
      } else {
        params.position = window
          .getComputedStyle(this.inputN.parentNode)
  ...
  prepareUpload() {
    ...
    if (
      this.fileSizeLimit === Number.MAX_SAFE_INTEGER &&
      this.fileSizeLimitDelegate !== undefined
    ) {
    ...
    for (let i = this.filesLength; i-- > 0; ) {
      ...
      if (this.files[i].size > this.maxSize) {
        if (this.onExceedLimit !== null) {
          ...
          console.warn(
            `${this.files[i].name} exceeds file size limit (${this.maxSize} bytes)`
          );
        }
        error = true;
      }
  ...
  ajaxUploadURLEncodedByEvent(e) {
    ...
    const filename = this.files[this.filePointer].name;
    const fileType =
      this.files[this.filePointer].type !== ""
        ? this.files[this.filePointer].type
  ...
  ajaxUploadURLEncoded(base64String, filename, fileType) {
    ...
    const queryIndex = this.controller.indexOf("?");
    const queryString =
      queryIndex !== -1 ? `&${this.controller.substring(queryIndex + 1)}` : "";
    ...
    if (this.onBeforeSend !== null) {
      ...
      const resultForSend =
        startIndex === -1 ? base64String : base64String.substring(startIndex);

      this.onBeforeSend(resultForSend, processedResultForSend => {
        const prefix =
          startIndex === -1 ? "" : base64String.substring(0, startIndex);
  ...
  initProgressWindow() {
    ...
    this.progressWindowN.setAttribute(
      "style",
      "width:100%; height:100%; display:table; position:fixed; left:0; top:0; font-size:16px; font-weight:400; transition:opacity 0.25s; background:rgba(0, 0, 0, 0.6); z-index:1048576;"
    );
    // eslint-disable-next-line prefer-template
    this.progressWindowN.innerHTML =
      '<div style="display:table-cell; text-align:center; vertical-align:middle;">\
                                                <div style="width:50%; box-sizing:border-box; border-radius:4px; padding:32px; margin:auto; background:' +
      this.bgColorCard +
      '; box-shadow:0 0 16px 0 #333;">\
                                                    <div style="margin-bottom:32px;">' +
      this.uploadLabel +
      '</div>\
                                                    <div style="line-height:32px; position:relative; color:#fff; border-radius:4px; margin-bottom:16px; background:' +
      this.bgColorIndicator +
      '; overflow:hidden;">\
                                                        <span id="globalGrogressBarLabel" style="position:relative; z-index:1;"></span>\
                                                        <div id="globalProgressBar" style="width:0; height:100%; position:absolute; left:0; top:0; background:' +
      this.bgColor +
      ';"></div>\
                                                    ...
                                                    <div style="line-height:32px; position:relative; color:#fff; border-radius:4px; background:' +
      this.bgColorIndicator +
      '; overflow:hidden;">\
                                                        <span id="localProgressBarLabel" style="position:relative; z-index:1;"></span>\
                                                        <div id="localProgressBar" style="width:0; height:100%; position:absolute; left:0; top:0;  background:' +
      this.bgColor +
      ';"></div>\
                                                        <div style="width:100%; height:100%; position:absolute; left:0; top:0; border-radius:inherit; box-shadow:inset 0px 0px 6px 0 #222"></div>\
                                                    </div>\
    ...
    this.globalProgressBarLabelN = document.getElementById(
      "globalGrogressBarLabel"
    );
    ...
    this.localProgressBarLabelN = document.getElementById(
      "localProgressBarLabel"
    );
