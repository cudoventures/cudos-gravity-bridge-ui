...
export default class TableMobile extends React.Component<
  TableMobileProps,
  State
> {
        ...
        <div className={"TableCell"}>
          ...
          <div onClick={this.onToggleSortDropDown}>
            {sortOptions.length > 0 && (
              <>
                {tableSortIndex === S.NOT_EXISTS && "Sort by"}
  ...
  renderRows() {
    ...
    const itemsSize =
      this.props.itemsSize === S.NOT_EXISTS
        ? this.props.legend.length
        : this.props.itemsSize;
    const resultRows = [];
    ...
    for (let i = 0; i < rows.length; ++i) {
      ...
      const resultRow = [
        <div key={0} className={"TableCell FlexRow"}>
      ...
      for (let j = 1; j < itemsSize; ++j) {
        resultRow.push(
          <div key={j} className={"TableCell FlexRow"}>
      ...
      resultRows.push(
        <div
          key={i}
          className={`TableRow Transition ${rows[i].rowClassName} `}
