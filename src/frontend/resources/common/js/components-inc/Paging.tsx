...
export default class Paging extends React.Component<Props> {
  render() {
    ...
    const totalPages = Number(
      Math.floor(
        (tableState.total + (tableState.itemsPerPage - 1)) /
    ...
    if (cntPage - 1 >= 0) {
      result.push(
        this.renderPageNode(-1, this.renderPreviousNode(), false, cntPage - 1)
      );
    }
    ...
    if (cntPage + pageOffset + 1 < totalPages) {
      ...
      result.push(
        this.renderPageNode(totalPages - 1, totalPages, false, totalPages - 1)
      );
    }

    if (cntPage + 1 < totalPages) {
      result.push(
        this.renderPageNode(-5, this.renderNextNode(), false, cntPage + 1)
      );
    }

    return <div className={"Paging"}> {result} </div>;
  }

  renderPreviousNode() {
    return (
      <div className={"SVG Size IconArrow"}>
        <SvgArrowLeft />
      </div>
    );
  }
  ...
  renderPageNode(
    key: number,
    text: string | React.ReactNode | number,
    active: boolean,
    page: number
  ) {
    const onClickHandler =
      page === S.NOT_EXISTS
        ...
        : () => {
            const helper = this.props.helper;
            helper.updateTablePage(
              page * this.props.helper.tableState.itemsPerPage
            );
          };
