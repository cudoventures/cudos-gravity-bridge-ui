...
class TableDesktop extends React.Component<TableDesktopProps> {
  ...
  render() {
    return (
      <div
        className={`Table TableDesktop ${S.CSS.getClassName(this.props.contentScrollable, "ContentScrollable")} ${this.props.className}`}
      >
        {this.renderLegend()}
        {this.renderRows()}
  ...
  renderLegend() {
    ...
    return <div className={"TableRow Legend"}> {legendRow} </div>;
  }
  ...
  renderRows() {
    ...
    if (rows.length === 0) {
      return <div className={"Empty FlexSingleCenter"}> No results found </div>;
    }
    ...
    for (let i = 0; i < rows.length; ++i) {
      ...
      for (let j = 0; j < cells.length; ++j) {
        resultRow.push(
          <div
            key={j}
            className={`TableCell FlexRow ${this.getCellAlign(j)} ${S.CSS.getClassName(tableSortIndex === j, "Sorted")}`}
            style={this.getCellStyle(j)}
          >
            {" "}
            {cells[j].content}{" "}
          </div>
      ...
      resultRows.push(
        <div
          key={i}
          className={`TableRow Transition ${row.rowClassName} `}
