...
export default class Table extends React.Component<Props> {
  ...
  static cellString(content: string, className = S.Strings.EMPTY) {
    const cellNode = (
      <span className={`Dots ${className}`} title={content}>
        {" "}
        {content}{" "}
      </span>
    );
...
export class TableBool extends React.Component<TableBoolProps> {
  ...
  render() {
    return (
      <div
        className={`TableBool ${this.props.className} ${S.CSS.getActiveClassName(this.props.value === S.INT_TRUE)}`}
      >
        {this.props.value === S.INT_TRUE
